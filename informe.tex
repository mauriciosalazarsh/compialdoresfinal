...
\begin{tikzpicture}[
    ...
    % Nodos principales en linea horizontal
    \node[io] (source) {Codigo Fuente\\(.c)};
    \node[block, right=1.2cm of source] (scanner) {Scanner\\(Lexico)};
    \node[block, right=1.2cm of scanner] (parser) {Parser\\(Sintactico)};
    \node[block, right=1.2cm of parser] (semantic) {Semantic\\Analyzer};
    \node[block, right=1.2cm of semantic] (codegen) {Code\\Generator};
    \node[io, right=1.2cm of codegen] (output) {Codigo x86-64\\(.s)};

    % Flechas con etiquetas
    \draw[arrow] (source) -- (scanner) node[label, midway, above] {texto};
    \draw[arrow] (scanner) -- (parser) node[label, midway, above] {tokens};
    \draw[arrow] (parser) -- (semantic) node[label, midway, above] {AST};
    \draw[arrow] (semantic) -- (codegen) node[label, midway, above] {AST};
    \draw[arrow] (codegen) -- (output) node[label, midway, above] {asm};

    % Caja de fondo para las fases del compilador
    \begin{scope}[on background layer]
        \node[draw=gray, dashed, rounded corners=5pt, fill=gray!5,
              fit=(scanner)(parser)(semantic)(codegen),
              inner sep=10pt, label={[font=\small]above:Fases del Compilador}] {};
    \end{scope}
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    root/.style={base, fill=red!20, text width=2cm, font=\footnotesize\ttfamily\bfseries},
    abstract/.style={base, fill=orange!20, text width=1.8cm},
    concrete/.style={base, fill=blue!15, text width=2cm},
    arrow/.style={->, thick, >=stealth}
]
    % Nodo raiz
    \node[root] (node) {ASTNode};

    % Nivel 1: Expr y Stmt
    \node[abstract, below left=1cm and 2cm of node] (expr) {Expr};
    \node[abstract, below right=1cm and 2cm of node] (stmt) {Stmt};

    % Nivel 2: Expresiones
    \node[concrete, below left=0.8cm and 1cm of expr] (binary) {BinaryExpr};
    \node[concrete, below=0.8cm of expr] (unary) {UnaryExpr};
    \node[concrete, below right=0.8cm and 1cm of expr] (ternary) {TernaryExpr};

    % Nivel 3: Mas expresiones
    \node[concrete, below=0.6cm of binary] (literal) {LiteralExpr};
    \node[concrete, below=0.6cm of unary] (ident) {IdentifierExpr};
    \node[concrete, below=0.6cm of ternary] (call) {CallExpr};

    % Nivel 2: Statements
    \node[concrete, below left=0.8cm and 0.8cm of stmt] (vardecl) {VarDeclStmt};
    \node[concrete, below=0.8cm of stmt] (ifstmt) {IfStmt};
    \node[concrete, below right=0.8cm and 0.8cm of stmt] (whilestmt) {WhileStmt};

    % Nivel 3: Mas statements
    \node[concrete, below=0.6cm of vardecl] (assign) {AssignStmt};
    \node[concrete, below=0.6cm of ifstmt] (forstmt) {ForStmt};
    \node[concrete, below=0.6cm of whilestmt] (block) {BlockStmt};
    \node[concrete, below=0.6cm of block] (ret) {ReturnStmt};

    % Flechas de herencia (hijo -> padre)
    ...
    % Leyenda
    \node[root, below=3.5cm of node, xshift=-4cm, minimum width=1.5cm] (leg1) {};
    \node[right=0.1cm of leg1, font=\footnotesize] {Clase base};
    \node[abstract, right=2cm of leg1, minimum width=1.5cm] (leg2) {};
    \node[right=0.1cm of leg2, font=\footnotesize] {Abstracta};
    \node[concrete, right=2cm of leg2, minimum width=1.5cm] (leg3) {};
    \node[right=0.1cm of leg3, font=\footnotesize] {Concreta};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    pointer/.style={->, thick, >=stealth, line width=1pt}
]
    % Celdas del stack
    \node[high] (arg) at (0, 0) {Argumentos (arg3, arg2, arg1)};
    \node[high] (ret) at (0, -0.9) {Direccion de retorno};
    \node[mid] (rbp) at (0, -1.8) {RBP anterior (saved rbp)};
    \node[low] (local) at (0, -2.7) {Variables locales};
    \node[low] (temp) at (0, -3.6) {Espacio temporal};

    % Punteros
    \draw[pointer] (3, -1.8) -- (2.6, -1.8);
    \node[right] at (3, -1.8) {\texttt{rbp}};

    \draw[pointer] (3, -3.6) -- (2.6, -3.6);
    \node[right] at (3, -3.6) {\texttt{rsp}};

    % Offsets
    \node[left, font=\footnotesize\ttfamily] at (-2.8, 0) {[rbp+24]};
    \node[left, font=\footnotesize\ttfamily] at (-2.8, -0.9) {[rbp+8]};
    \node[left, font=\footnotesize\ttfamily] at (-2.8, -1.8) {[rbp]};
    \node[left, font=\footnotesize\ttfamily] at (-2.8, -2.7) {[rbp-8]};
    \node[left, font=\footnotesize\ttfamily] at (-2.8, -3.6) {[rbp-16]};

    % Etiquetas de direcciones
    \node[above] at (0, 0.6) {\textbf{Direcciones altas}};
    \node[below] at (0, -4.2) {\textbf{Direcciones bajas}};

    % Flecha de crecimiento
    \draw[->, thick, dashed] (-4, 0) -- (-4, -3.6);
    \node[rotate=90, font=\footnotesize] at (-4.4, -1.8) {Crece hacia abajo};
\end{tikzpicture}
