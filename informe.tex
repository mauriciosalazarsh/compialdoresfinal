...
\begin{tikzpicture}[
    node distance=0.5cm,
    base/.style={rectangle, draw=black, line width=0.8pt, minimum height=0.7cm, text centered, font=\footnotesize\ttfamily, rounded corners=2pt},
    root/.style={base, fill=red!20, text width=1.8cm},
    abst/.style={base, fill=orange!20, text width=1.5cm},
    leaf/.style={base, fill=blue!15, text width=1.8cm},
    myarrow/.style={-stealth, thick}
]
    \node[root] (node) {ASTNode};

    \node[abst, below left=0.8cm and 1.5cm of node] (expr) {Expr};
    \node[abst, below right=0.8cm and 1.5cm of node] (stmt) {Stmt};

    \node[leaf, below left=0.6cm and 0.3cm of expr] (binary) {BinaryExpr};
    \node[leaf, below=0.6cm of expr] (unary) {UnaryExpr};
    \node[leaf, below right=0.6cm and 0.3cm of expr] (ternary) {TernaryExpr};
    \node[leaf, below=0.5cm of binary] (literal) {LiteralExpr};
    \node[leaf, below=0.5cm of ternary] (call) {CallExpr};

    \node[leaf, below left=0.6cm and 0.3cm of stmt] (vardecl) {VarDeclStmt};
    \node[leaf, below=0.6cm of stmt] (ifstmt) {IfStmt};
    \node[leaf, below right=0.6cm and 0.3cm of stmt] (whilestmt) {WhileStmt};
    \node[leaf, below=0.5cm of vardecl] (forstmt) {ForStmt};
    \node[leaf, below=0.5cm of whilestmt] (block) {BlockStmt};

    \draw[myarrow] (expr) -- (node);
    \draw[myarrow] (stmt) -- (node);
    \draw[myarrow] (binary) -- (expr);
    \draw[myarrow] (unary) -- (expr);
    \draw[myarrow] (ternary) -- (expr);
    \draw[myarrow] (literal) -- (expr);
    \draw[myarrow] (call) -- (expr);
    \draw[myarrow] (vardecl) -- (stmt);
    \draw[myarrow] (ifstmt) -- (stmt);
    \draw[myarrow] (whilestmt) -- (stmt);
    \draw[myarrow] (forstmt) -- (stmt);
    \draw[myarrow] (block) -- (stmt);
\end{tikzpicture}
