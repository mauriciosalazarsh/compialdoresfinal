...
void CodeGenerator::generatePrologue(const std::string& func,
    ...
    if (alignedSize > 0) {
        emit("subq $" + std::to_string(alignedSize) + ", %rsp");
    }
}
...
void CodeGenerator::visit(BinaryExpr* node) {
    ...
    emit("pushq %rax");  // Guardar operando izquierdo

    node->right->accept(this);
    emit("movq %rax, %rbx");  // Operando derecho en rbx
    emit("popq %rax");        // Operando izquierdo en rax
    ...
    if (node->op == "+") {
        ...
        emit("idivq %rbx");   // Resultado en rax
    } else if (node->op == "%") {
        emit("cqto");
        emit("idivq %rbx");
        emit("movq %rdx, %rax"); // Resto en rdx
    }
}
\end{lstlisting}
...
void CodeGenerator::visit(BinaryExpr* node) {
    if (enableConstantFolding) {
        ...
        if (leftLit && rightLit &&
            ...
            emit("movq $" + std::to_string(result) + ", %rax");
            return;
        }
    }
}
...
movq $25, %rax          ; En lugar de multiples operaciones
\end{lstlisting}
...
\begin{lstlisting}[style=asm, caption=Codigo x86-64 generado (Sintaxis AT\&T)]
.text
.global main
...
factorial:
    pushq %rbp
    movq %rsp, %rbp
    movq 16(%rbp), %rax      ; Cargar parametro n
    pushq %rax
    movq $1, %rax
    movq %rax, %rbx
    popq %rax
